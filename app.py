...
with tab2:
    ...
    if st.button('Test Result'):
        ...
        st.markdown(f"### :green[Tool Condition:{["Worn" if p[0] > 0.2 else "Unworn" for p in prediction]}]")
        st.markdown(f"### :green[Machine Finalized:{["Completed" if p[1] > 0.2 else "Not Completed" for p in prediction]}]")
        st.markdown(f"### :green[Visual Inspection:{["Inspection Passed" if p[2] > 0.2 else "Inspection Failed" for p in prediction]}]")
